...
class DAVariant:
    def __init__(self, n_keypoints=30, patch_size=40, n_consecutive_frames=5, hidden_layer_dimension=2500,
        ...
        '''
        Build the modified loss function
        '''
    ...
    def fit(self, x):
        ...
        # RUN
        init = tf.global_variables_initializer()  # check if I should do this every call of the fit function
        with self.sess as sess:
            sess.run(init)  # check if I should do this every call of the fit function
            for step in range(self.n_epochs):
                sess.run([self.loss, train_fn], feed_dict={self.x_placeholder: x})
